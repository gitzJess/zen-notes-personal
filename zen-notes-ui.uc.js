...
(function () {
  ...
  function createWidget() {
    ...
    /* ── Header ──────────────────────────────────────────────── */
    const header = createXHTMLElement("div");
    header.className = "zen-notes-header";
    header.setAttribute("align", "center");
    const collapsedButton = createXHTMLElement("button");
    ...
    const notePicker = createXHTMLElement("select");
    ...
    function updateCountLabel() {
      ...
      countLabel.textContent = `${words}w`;
    }
    ...
    function flushPendingSave() {
      ...
      const { html, noteId } = pendingSave;
      const sanitized = sanitizeHTML(html);
      const note = state.notes.find(n => n.id === noteId);
      if (note && note.contentHTML !== sanitized) {
        const previous = { ...note };
        ...
        try { persistState(state); }
          ...
          setSaveStatus("Save failed — keep this window open");
          if (saveStatusClearTimer) clearTimeout(saveStatusClearTimer);
          throw error;
        }
      }
    ...
    function scheduleSave(html) {
      ...
      pendingSave = { html, noteId: state.activeNoteId };
    ...
    function flushCurrentEditorImmediately() {
      if (!state.note) return;
      pendingSave = { html: editor.innerHTML || "", noteId: state.activeNoteId };
      flushPendingSave();
    }
    ...
    function renderAll() {
      ...
      notePicker.value = state.activeNoteId;
    ...
    function hideToolbar() {
      if (toolbar.hidePopover && toolbar.matches(":popover-open")) toolbar.hidePopover();
      toolbar.hidden = true;
    }
    function positionToolbar() {
      ...
      toolbar.style.setProperty("--notes-toolbar-y", `${Math.max(4, Math.min(
        top >= bounds.top ? top : rect.bottom + 8, window.innerHeight - size.height - 4))}px`);
    }
    ...
    /* Dragging uses a small collapsed stop and a generous expansion threshold. */
    ...
    const prefObserver = { observe(subject, topic, data) {
      if (topic === "nsPref:changed") {
        ...
        if (data === PREF_PRESET) applyColorMode();
      }
    } };
