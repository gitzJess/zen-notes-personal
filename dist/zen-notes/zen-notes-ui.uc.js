...
(function () {
  ...
  function createWidget() {
    ...
    noteControls.appendChild(notePicker);
    ...
    notePicker.addEventListener("change", () => chooseNote(notePicker.value));
    noteAction("+ New", () => {
      flushCurrentEditorImmediately();
    ...
    noteAction("Rename", () => {
      const title = window.prompt("Note name:", state.note.title);
      if (title === null || !title.trim()) return;
      flushCurrentEditorImmediately();
      state.note.title = title.trim().slice(0, 120);
      persistState(state);
      renderAll();
    });
    noteAction("Delete", () => {
