...
(function () {
  ...
  const VERSION = "2.5.0-personal.1";
  ...
  function persistState(state) {
    // The active note is a runtime alias, never a second stored copy.
    const { note, ...stored } = state;
    Services.prefs.setStringPref(PREF_DATA, JSON.stringify(stored));
  ...
  function loadState(initialWorkspaceId) {
    ...
    if (state.version !== SCHEMA_VERSION || !Array.isArray(state.notes)) {
      if (serialized) setPrefString(PREF_DATA_BACKUP, serialized);
      const candidates = Array.isArray(state.notes) ? state.notes : state.note ? [state.note] : [];
      state = { version: SCHEMA_VERSION, notes: candidates, activeNoteId: state.note?.id,
        lastWorkspaceId: state.lastWorkspaceId || initialWorkspaceId };
    }
    const ids = new Set();
    state.notes = state.notes.filter(n => n && typeof n === "object").map(n => {
      const note = createNote(typeof n.title === "string" ? n.title : "Untitled note", {
        ...n, contentHTML: typeof n.contentHTML === "string" ? n.contentHTML : ""
      });
      if (typeof note.id !== "string" || ids.has(note.id)) note.id = createId("note");
      ids.add(note.id);
      return note;
    });
    if (!state.notes.length) state.notes.push(createInitialV4State(initialWorkspaceId).note);
    state.activeNoteId = state.notes.find(n => n.id === state.activeNoteId)?.id || state.notes[0].id;
    Object.defineProperty(state, "note", { configurable: true, enumerable: false,
      get() { return this.notes.find(n => n.id === this.activeNoteId); } });
    persistState(state);
    let changed = false;
